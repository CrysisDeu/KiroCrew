...
After the manager boundary, an execution or control exception is rethrown only
when its rejected command outcome commits. A failed or rejected settlement
surfaces as typed outcome uncertainty, because the legacy side effect may have
occurred and an exact retry is not safe.
...
request only for `PENDING`, while `CLAIMED` remains outcome-uncertain and is
