...
// answer one question. Once a crash dump has reported `peakKeysForOneName`, the
...
function normalizeWindow(w) {
  ...
  return {
    calls,
    keys: num(w.keys),
    maxKeysForOneName: num(w.maxKeysForOneName),
...
function renderWindow(entry) {
  ...
  return (
    ...
    `maxKeysForOneName=${w.maxKeysForOneName} repeatKeyCalls=${w.repeatKeyCalls} ` +
 ...
 *
 *  `peakKeysForOneName` is the number that decides the hypothesis: >1 means a
...
function renderSummary(entries) {
  ...
  for (const e of entries) {
    ...
    if (w.maxKeysForOneName > peakKeysForOneName) peakKeysForOneName = w.maxKeysForOneName;
  ...
  return (
    ...
    `calls=${calls} keys=${keys} peakKeysForOneName=${peakKeysForOneName} ` +
