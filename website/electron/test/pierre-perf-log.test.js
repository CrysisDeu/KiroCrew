...
test("non-finite fields are coerced rather than reaching a log line", () => {
  ...
  assert.equal(w.maxKeysForOneName, 0);
...
test("flush leads with a summary carrying the peak ratio", () => {
  ...
  log.record(win({ calls: 1, keys: 1, maxKeysForOneName: 1, chars: 100, maxLen: 100 }));
  log.record(
    win({ calls: 4, keys: 4, maxKeysForOneName: 4, chars: 1000, maxLen: 200, heapMB: 900 })
  );
  const lines = log.flush();
  assert.match(lines[0], /pre-crash summary over 2 window\(s\)/);
  assert.match(lines[0], /peakCharsPerMaxLen=5\.0/);
  assert.match(lines[0], /peakKeysForOneName=4/);
...
test("the summary distinguishes many-blocks-once from one-block-churned", () => {
  ...
  // 5 times. peakKeysForOneName is the only field that separates them, which is
  // why the verdict is read off it and not off the ratio.
  const many = createPierrePerfLog({ now: () => "T" });
  many.record(win({ calls: 5, keys: 5, maxKeysForOneName: 1, chars: 500, maxLen: 100 }));
  const churn = createPierrePerfLog({ now: () => "T" });
  churn.record(win({ calls: 5, keys: 5, maxKeysForOneName: 5, chars: 500, maxLen: 100 }));
  ...
  assert.match(manyLine, /peakKeysForOneName=1/);
  assert.match(churnLine, /peakKeysForOneName=5/);
});

test("a memoization miss is reported apart from key churn", () => {
  const log = createPierrePerfLog({ now: () => "T" });
  // Same content re-hashed 6 times: no new keys, so this is NOT churn.
  log.record(win({ calls: 7, keys: 1, maxKeysForOneName: 1, repeatKeyCalls: 6 }));
  const lines = log.flush();
  assert.match(lines[0], /peakRepeatKeyCalls=6/);
  assert.match(lines[0], /peakKeysForOneName=1/);
