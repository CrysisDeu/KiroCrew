 ...
 *
 * The allow-list here is DERIVED from tailwind.config.js rather than hardcoded,
 * so renaming or dropping a theme token fails this test instead of silently
 * reintroducing an invisible dot.
 */
...
const tailwindSrc = readFileSync(resolve(__dirname, '../../tailwind.config.js'), 'utf-8')
...
function themeColorTokens(): Set<string> {
  ...
  expect(block, 'could not locate theme.extend.colors in tailwind.config.js').toBeTruthy()
  const tokens = new Set<string>()
  for (const m of block![1].matchAll(/^\s*'?([a-z0-9-]+)'?:\s/gm)) tokens.add(m[1])
  return tokens
}
...
describe('remote-crew status dot colors', () => {
  it('maps every tunnel state to a color token the theme defines', () => {
    ...
    // Floor: a broken regex that matched nothing would make the loop below pass vacuously.
