...
describe('pierrePerf accounting', () => {
  ...
  it('records nothing at all when no reporter exists', () => {
    ...
    // ever drains the counters. If accounting still ran, keysByName would retain
    ...
    setApi(undefined)
    startPierrePerfReporting()
    for (let i = 0; i < 50; i++) recordCacheKey('f.ts', `f.ts:${i}:h`, i)
  ...
  it('sums calls, chars and the largest content length', () => {
    recordCacheKey('a.ts', 'a.ts:3:x', 3)
    recordCacheKey('b.ts', 'b.ts:10:y', 10)
    const w = drainWindow()
  ...
  it('counts distinct keys, so repeated identical renders are not miscounted as churn', () => {
    ...
    recordCacheKey('a.ts', 'a.ts:5:same', 5)
    recordCacheKey('a.ts', 'a.ts:5:same', 5)
    recordCacheKey('a.ts', 'a.ts:5:same', 5)
    const w = drainWindow()
    ...
    expect(w?.maxKeysForOneName).toBe(1)
  })
  ...
  it('exposes the quadratic signature of a streamed block', () => {
    ...
    for (const len of [10, 20, 30, 40]) recordCacheKey('f.ts', `f.ts:${len}:h`, len)
    const w = drainWindow()
    ...
    expect(w?.maxKeysForOneName).toBe(4)
  ...
  it('does NOT report churn when many distinct blocks are each rendered once', () => {
    ...
    // so the aggregate ratio cannot tell the two apart. maxKeysForOneName can.
    for (const len of [10, 20, 30, 40]) recordCacheKey(`f${len}.ts`, `f${len}.ts:${len}:h`, len)
    const w = drainWindow()
  ...
  it('attributes churn to the busiest name when blocks are interleaved', () => {
    // A streaming block alongside quiet ones: the verdict must follow the block
    // that actually churned, not the count of names.
    recordCacheKey('quiet.ts', 'quiet.ts:5:a', 5)
    recordCacheKey('other.ts', 'other.ts:5:b', 5)
    for (const len of [10, 20, 30]) recordCacheKey('busy.ts', `busy.ts:${len}:h`, len)
    const w = drainWindow()
    expect(w?.keys).toBe(5)
    expect(w?.maxKeysForOneName).toBe(3)
  })

  it('drains, so each window is independent', () => {
    recordCacheKey('a.ts', 'a.ts:1:x', 1)
    drainWindow()
    expect(drainWindow()).toBeNull()
  })
...
describe('pierrePerf reporting lifecycle', () => {
  ...
  it('reports a non-empty window on the interval', () => {
    ...
    setApi(spy)
    startPierrePerfReporting()
    recordCacheKey('f.ts', 'f.ts:10:h', 10)
    recordCacheKey('f.ts', 'f.ts:20:h', 20)
  ...
  it('survives a throwing reporter — diagnostics never break rendering', () => {
    ...
    recordCacheKey('f.ts', 'f.ts:11:h', 11)
    expect(() => vi.advanceTimersByTime(5000)).not.toThrow()
