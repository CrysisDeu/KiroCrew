...
describe('ServicesTab channel rows', () => {
  ...
  it('renders a row for every channel the gateway can report', async () => {
    ...
    // connected or failing. That is how WhatsApp was missed: the map ended at
    // imessage. Asserted per channel rather than as a count, since a count passes
    // just as happily when one name is swapped for another.
    const CHANNELS = [
      ['slack', 'Slack'],
      ['telegram', 'Telegram'],
      ['discord', 'Discord'],
