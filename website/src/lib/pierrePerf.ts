...
//
// So the falsifiable signal is `maxKeysForOneName` — how many distinct keys ONE
// file name minted inside a window. Streaming a block in C chunks drives it to C;
// rendering C different blocks once each leaves it at 1. The aggregate ratio
// `chars / maxLen` deliberately does NOT carry the verdict: five distinct blocks
// rendered once produce the same ratio and the same `keys == calls` shape as one
// block re-tokenized five times, so reading confirmation off the ratio alone would
// false-confirm on an ordinary multi-block chat window. The ratio still bounds how
// much hashing was wasted; the per-name count is what names the cause.
//
...
export interface PierrePerfWindow {
  ...
  chars: number
  /** Distinct keys minted across ALL names. Each new key is a forced re-tokenize,
   *  so this is the worker-side task count the pool actually had to run. */
  keys: number
  /** Distinct keys minted for the single busiest NAME — the churn verdict.
   *
   *  This exists because the aggregate numbers cannot tell the two shapes apart:
   *  one block streamed in 5 chunks and 5 distinct blocks each rendered once both
   *  produce `keys == calls` and `chars/maxLen > 1`. Only a count scoped to ONE
   *  name separates them — the streamed block reaches 5 here, while five
   *  independent blocks stay at 1 each. Confirming the re-tokenize hypothesis
   *  reads THIS field, not the ratio. */
  maxKeysForOneName: number
  /** Calls whose key had already been seen for that name in this window: the same
   *  content re-hashed with no content change. That is a memoization failure, a
   *  DIFFERENT defect from key churn, and separating them keeps one dump from
   *  being read as evidence for the wrong one. */
  repeatKeyCalls: number
  /** Longest single content seen. `chars / maxLen` bounds the wasted hashing, but
   *  it is NOT the churn verdict on its own — see maxKeysForOneName. */
  maxLen: number
...
// empty `keysByName` and it would retain every key for the life of the session.
...
let enabled = false
// Keys are grouped BY NAME, because the number that answers the question is
// "how many times was ONE block re-tokenized", not "how many keys existed".
// Discarded every window, so it cannot grow without bound the way a
// session-lifetime registry would.
let keysByName = new Map<string, Set<string>>()
 ...
 *
 *  `name` is passed separately rather than parsed back out of the key because the
 *  key embeds it with a `:` separator and a file name may itself contain one. */
export function recordCacheKey(name: string, key: string, length: number): void {
  if (!enabled) return
  calls += 1
  chars += length
  if (length > maxLen) maxLen = length
  let seen = keysByName.get(name)
  if (!seen) {
    seen = new Set<string>()
    keysByName.set(name, seen)
  }
...
export function drainWindow(): PierrePerfWindow | null {
  ...
  let keys = 0
  let maxKeysForOneName = 0
  for (const seen of keysByName.values()) {
    keys += seen.size
    if (seen.size > maxKeysForOneName) maxKeysForOneName = seen.size
  }
  ...
  maxLen = 0
  repeatKeyCalls = 0
  keysByName = new Map<string, Set<string>>()
  return snapshot
}
