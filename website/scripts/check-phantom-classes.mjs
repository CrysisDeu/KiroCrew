...
/** One or more leading Tailwind variant segments (`hover:`, `data-[x=y]:`, …). */
...
/** The candidate class tokens in one literal value. */
...
export function candidatesIn(src) {
  ...
  for (const entry of literals(src)) {
    ...
    const tokens = tokensOf(entry.value)
...
export function violationsIn(src, emitted, rel) {
  ...
  for (const { tokens, line, group, before } of entries) {
    const bad = tokens.filter((t) => !emitted.has(t))
    ...
    for (const token of bad) out.push({ path: rel, line, token })
  }
  return out
}
...
async function scanAll() {
  ...
  const emitted = await emittedClasses(
    candidates.filter((t) => /^[a-z0-9@[]/i.test(t) && t.length < 80),
  )
...
async function selfTest() {
  ...
  for (const p of PROBES) {
    const got = violationsIn(p.src, emitted, 'probe.tsx').map((v) => v.token)
    const want = p.caught
    if (got.join(',') !== want.join(',')) {
      disagree.push(
        `${p.name}: expected [${want.join(', ')}], got [${got.join(', ')}]`,
      )
    }
  }
