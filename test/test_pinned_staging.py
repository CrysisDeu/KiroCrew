...
@pinned_only
def test_a_filesystem_without_hardlinks_still_publishes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """FAT/exFAT has no hard links, and that must not crash a restore.

    Publishing by link is what makes no-clobber atomic, but a filesystem that refuses links
    would raise `EPERM` from the publish -- and on a replace restore that happens AFTER the
    live file was moved into the backup, so the original is gone from its place and nothing
    is put back. Review found it.

    Answered by falling back to rename, NOT by refusing: the base branch restores to such a
    filesystem today, and removing a working platform to satisfy a property is the same
    mistake as refusing the dashboard import on Windows, already corrected once here.
    """
    src_dir = tmp_path / "src"
    src_dir.mkdir()
    (src_dir / "f.txt").write_text("payload\n", encoding="utf-8")
    ...
    def _no_hardlinks(*a, **k):
        raise OSError(errno.EPERM, "operation not permitted")

    monkeypatch.setattr(pinned_fs.os, "link", _no_hardlinks)
    assert pinned_fs.copy_file_pinned(str(src_dir / "f.txt"), str(dst_dir / "f.txt")) is True

    assert (dst_dir / "f.txt").read_text(encoding="utf-8") == "payload\n"
    names = sorted(p.name for p in dst_dir.iterdir())
    assert names == ["f.txt"], f"temporary left behind on the fallback path: {names}"
