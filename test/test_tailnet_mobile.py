    ...
    *,
    ...
    class _Req:
        def __init__(self) -> None:
            self.app = {"port": port, "state": SimpleNamespace(owner_id="owner@example.com")}
...
class TestQrRefusals:
    ...
    async def test_no_tailnet_name_refuses(self, _unrestricted, _quiet_audit) -> None:
        with patch.object(tailnet_mobile.tailnet, "self_dns_name", return_value=None):
            resp = await tailnet_mobile.api_tailnet_mobile_qr(_request())
    ...
    async def test_unpublished_refuses_before_minting(self, _unrestricted, _quiet_audit) -> None:
        ...
        state = SimpleNamespace(published=False, configured=True, detail="not ours")
        with (
            patch.object(tailnet_mobile.tailnet, "self_dns_name", return_value=_HOST),
            patch.object(tailnet_mobile.tailnet_serve, "serve_state", return_value=state),
    ...
    async def test_undetermined_serve_state_also_refuses(self, _unrestricted, _quiet_audit) -> None:
        state = SimpleNamespace(published=None, configured=None, detail="unreadable")
        with (
            patch.object(tailnet_mobile.tailnet, "self_dns_name", return_value=_HOST),
            patch.object(tailnet_mobile.tailnet_serve, "serve_state", return_value=state),
        ):
    ...
    async def test_ttl_cannot_be_talked_past_the_endpoint_ceiling(
        ...
        with (
            ...
            resp = await tailnet_mobile.api_tailnet_mobile_qr(_request(body={"ttl": "500h"}))
