    ...
    return web.json_response(
        {
            ...
            "trusted": trusted,
            "startup_trusted": bool(startup_host),
            "published": published,
            ...
            "detail": serve_detail or probe.detail,
...
async def api_tailnet_mobile_qr(request: web.Request) -> web.Response:
    ...
    Refuses unless the dashboard is actually published — a QR for a URL nothing
    answers is a support ticket, not a feature.
    """
    refusal = _guard(request)
    if refusal is not None:
    ...
    port = _dashboard_port(request)
    host = await asyncio.to_thread(tailnet.self_dns_name)
    if not host:
        _audit(request, "tailnet.mobile.qr", "denied", "no-tailnet-name")
        return web.json_response(
            {
                "error": (
                    "This machine has no tailnet name right now, so there is "
    ...
    if state.published is not True:
        _audit(request, "tailnet.mobile.qr", "denied", "not-published")
        return web.json_response(
            {
                "error": (
                    "The dashboard is not published on the tailnet, so a phone "
                    f"could not reach it. {state.detail}"
                ),
                "code": "not_published",
            },
            status=409,
