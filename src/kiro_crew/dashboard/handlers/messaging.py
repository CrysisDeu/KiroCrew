...
from kiro_crew.messaging.renderer import chunk_for_transport, display_safe, format_overflow
...
async def _deliver_channel_dm(
    ...
    units = chunk_for_transport(display_safe(text), live_transport.capabilities)
    try:
        for unit in units:
...
async def _send_to_channel_target(
    ...
    *,
    ...
    session_key = caller_session or HOST_SESSION_KEY
    gov = _vet_channel_send(channel_type, session_key)
    if gov:
