...
def apply_owner_only(
    ...
    *,
    inherit: bool,
    sids: tuple[str, ...],
    require_local_volume: bool = False,
) -> None:
    ...
    *require_local_volume* is for a caller that cannot afford an unbounded wait --
    in practice, one running on the event loop. It refuses a UNC path or mapped
    network drive before any API call. It defaults to False so that a network-homed
    data home keeps the DACL the icacls path gave it; see the comment at the check.
    ...
    advapi32, kernel32 = _load()
    # The volume constraint belongs to the CALL SITE, not to this mechanism, which
    # is why it is a parameter and defaults to off.
    #
    # `SetNamedSecurityInfoW` on a UNC path or a mapped network drive is an SMB
    # round-trip: correct, but unbounded. That is only a hazard for a caller on the
    # asyncio event loop, and there is exactly one of those -- the inline Windows
    # branch of `write_config_atomically`, which passes `require_local_volume=True`.
    # Every other caller in the tree is a synchronous CLI path, a startup path, or
    # already offloaded via `asyncio.to_thread`, and for those an SMB round-trip is
    # simply a slow write.
    #
    # So the default APPLIES the DACL on a network volume, because refusing there
    # would be a security regression rather than a safeguard: the icacls path this
    # replaces did apply DACLs over SMB (SMB carries NTFS ACLs), so defaulting to a
    # refusal would silently downgrade a data home on a redirected or mapped
    # enterprise share from a real owner-only DACL to whatever it inherits.
    #
    # If you add a NEW caller that runs on the event loop, pass
    # `require_local_volume=True` -- or, better, offload it.
    if require_local_volume and not _volume_is_local(kernel32, Path(os.fspath(path))):
        raise AclWriteFailed(
            "refusing to apply a DACL on a non-local volume (UNC or mapped "
            "network drive) from a caller that requires a local volume: the write "
            "would be an unbounded SMB round-trip and this caller runs on the "
            "event loop"
        )
