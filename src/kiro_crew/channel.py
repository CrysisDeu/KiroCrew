...
async def _stream_task(
    ...
    # circular import: kiro_crew.channel is imported by the dashboard package,
    # so a module-level import of dashboard modules would be cyclic.
    from kiro_crew.dashboard.chat_utils import _extract_bash_command
    ...
    try:
        async for event in client.stream(message):
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                # populate only ``title``).
                _card_name = f"Running: {_cmd}" if _cmd else (event.text or event.title or "")
                ...
                agent._pending_approval_command = "" if "[REDACTED" in _cmd else _cmd
