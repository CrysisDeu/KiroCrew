...
def _apply_metadata(
    ...
    *,
    ...
    dst_name: str | None,
    mode: int | None = None,
    by_name_target: str | None = None,
) -> None:
    """Copy mode and timestamps onto the destination, by descriptor where possible.

    ``by_name_target`` is the name the FALLBACKS must use, and it is load-bearing on
    Windows. The bytes are written to a private temporary and published by link only after
    this returns, so on a platform without ``os.fchmod`` the by-name form has to name that
    temporary -- naming the final destination there would try to chmod a path that does not
    exist yet, and if it did exist it would belong to somebody else. The descriptor form is
    unaffected either way, which is why this only matters where the fd form is missing.
    ...
    *mode* overrides the source's mode. Used for a restored security file, which must end
    ...
    # The fallbacks name the TEMPORARY, never the final destination -- see the
    # `by_name_target` note above. `dst_name`/`dst` stay in the signature because the
    # directory-relative form still needs the descriptor, and because a caller that has no
    # temporary (none today) would fall back to them.
    fallback_name = by_name_target if by_name_target is not None else dst_name
    fallback_path = by_name_target if by_name_target is not None else (str(dst) if dst else None)
    if hasattr(os, "fchmod"):
