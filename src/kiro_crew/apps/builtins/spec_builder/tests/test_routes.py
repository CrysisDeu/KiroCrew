...
@pytest.mark.asyncio
async def test_a_turn_that_takes_the_slot_during_relay_restores_pending(
    tmp_path, monkeypatch
):
...
@pytest.mark.asyncio
async def test_irreversible_consumption_finalizes_before_the_turn_finishes(
    tmp_path, monkeypatch
):
...
@pytest.mark.asyncio
async def test_create_refuses_an_unreadable_index_before_orphan_cleanup(
    tmp_path, monkeypatch
):
