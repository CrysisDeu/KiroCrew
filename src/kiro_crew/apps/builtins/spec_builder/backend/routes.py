...
def _reservation_is_ours(meta: dict) -> bool:
    """True when a delete reservation is live here or durably destructive.
    ...
    return isinstance(held, dict) and (
        ...
        or held.get(_DELETE_TEARDOWN_COMMITTED) is True
    )
...
async def _mark_deleting(name: str, *, expect_spec_dir: str, expect_slot_key: str) -> bool:
    ...
    def _apply(index: dict) -> bool:
        ...
        committed = (
            ...
            and previous.get(_DELETE_TEARDOWN_COMMITTED) is True
        )
        meta[_DELETING] = {
            "owner": _PROCESS_ID,
            "at": time.time(),
...
async def _commit_delete_teardown(
    name: str, *, expect_spec_dir: str, expect_slot_key: str
) -> bool:
...
def _restore_decision_pending_locked(
    ...
    with _DECISIONS_LOCK:
        ...
        try:
            ...
            logger.warning(
                "could not restore undelivered decision for %s", spec_dir, exc_info=True
            )
            return False
        return True
            ...
            return web.json_response(
                {
                    ...
                    "error": (
                        "could not reserve this spec's destructive cleanup; retry "
                        "the delete"
                    ),
                },
                status=503,
