...
def snapshot_main(
    ...
    # Windows applies an owner-only DACL.
    ...
    for f in CORE_FILES.get(component, ()):
        ...
        if (snap / f).is_file():
            ...
            if component == "security":
                ...
                # chmod 0o600; Windows applies an owner-only DACL.
                # Unlink the freshly
    ...
    if _want(components, "security"):
        for f in CORE_FILES["security"]:
            ...
            if s.is_file() and not d.is_file():
                ...
                # applies an owner-only DACL. Unlink the freshly
                # copied file on
                # failure so a lockdown error doesn't leave a restored secret
