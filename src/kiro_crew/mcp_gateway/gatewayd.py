...
async def _apply_claim(
    ...
    for conn in list(conns):
        ...
        if pool is not None:
            # The stub is changing OWNER: its resource subscriptions belong
            # to the old principal, and without eviction the new session
            # would keep receiving the old session's resource-update URIs
            # (which can carry tokens or presigned params). Caller-binding
