...
def send_message(name: str, args: dict[str, Any]) -> str:
    ...
    # caller's OWN profile. Filtering to cron here left every non-cron caller
    # re-vetted at the gateway as ``HOST_SESSION_KEY`` -- a permissive default
    # that, paired with this module's fail-OPEN channel vet
    # (``_vet_channel_governance`` returns None on an evaluation error), let a
    # caller whose own profile denies the transport reach a host-permitted target.
